...
# TODO make bomb objects global here
...
def init_agents():
    ...
    agts[0].init_agent(10, constants.GameType.FFA)
    agts[1].init_agent(11, constants.GameType.FFA)
    agts[0].agent_id = 0
    agts[1].agent_id = 1
...
def game_state_from_obs(obs, own_agent_id):
    ...
    update_agents(obs["board"], obs["ammo"])

    game_state = [
        obs["board"],
        agts,
        convert_bombs(obs["board"], np.array(obs["bomb_blast_strength"]), np.array(obs["bomb_life"]),
                      np.array(obs["bomb_moving_direction"]), agts, own_agent_id),
...
def convert_bombs(board, strength_map, life_map, bomb_moving_direction, agents, own_id):
    ...
    for aid in [10, 11]:
        bomber_locations = np.where(board == aid)
        r = bomber_locations[0]
        c = bomber_locations[1]
        if strength_map[r, c] > 0:  # 0 & no bomb there yet:
            print("new bomb at %d %d by agents %d" % (r, c, aid - 10))
            bombs.append(characters.Bomb(agents[aid - 10], (r, c), int(life_map[(r, c)]), int(strength_map[(r, c)]),
                                         constants.Action(bomb_moving_direction[(r, c)])))

    # check existing bombs with strength map, consider moving and chain explosions

    for r, c in zip(locations[0], locations[1]):
        if bomber_id_map[r, c] == -1:
            # TODO add bomb life and kick
...
def update_agents(board, ammo):
    ...
    for aid in [10, 11]:
        ...
        agts[aid - 10].reset(ammo=ammo, is_alive=agts[10 - aid].is_alive)  # TODO ammo only for own agent
