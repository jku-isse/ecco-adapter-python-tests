...
from pommerman import agents, utility
...
def game_state_from_obs(obs, own_agent_id):
    ...
    game_state = [
        obs["board"],
        agts,
        convert_bombs(obs["board"], np.array(obs["bomb_blast_strength"]), np.array(obs["bomb_life"]), obs["flame_life"],
    ...
    if agts[own_agent_id].ammo != own_ammo:
        ...
        agts[own_agent_id].ammo = own_ammo
    ...
    if not check_bombs(obs["bomb_blast_strength"], obs["bomb_life"], obs["bomb_moving_direction"]):
        print("bomb update error")

    global previous_board
    previous_board = obs["board"]

    return game_state


def advance_bomb(bomb, board, strength_map, flame_life_map, bomb_moving_direction):
    if bomb.is_moving():

        (old_r, old_c) = bomb.position
        if strength_map[(old_r, old_c)] < 2:
            if flame_life_map[(old_r, old_c)] == 0:
                bomb.move()
                print("bomb moved to %d,%d" % (old_r, old_c))  # TODO check correct

    bomb.tick()

    (r, c) = bomb.position
    if bomb.life == 0 or flame_life_map[r, c] > 0:
        bomb.bomber.ammo += 1
        return False

    # init kick
    if strength_map[r, c] != bomb.blast_strength:
        kicker_id = board[r, c]
        location = np.where(previous_board == kicker_id)
        (r, c) = (2 * r - location[0][0], 2 * c - location[1][0])
        bomb.position = (r, c)

    # update moving direction
    d = int(bomb_moving_direction[r, c])
    bomb.moving_direction = (None if d == 0 else constants.Action(bomb_moving_direction[r, c]))

    return True


def convert_bombs(board, strength_map, bomb_life_map, flame_life_map, bomb_moving_direction):
    """ converts bomb matrices into bomb object list that can be fed to the forward model """

    # move and remove existing bombs
    bombs[:] = [bomb for bomb in bombs if
                advance_bomb(bomb, board, strength_map, flame_life_map, bomb_moving_direction)]
...
def check_bombs(strength_map, bomb_life_map, bomb_moving_direction):

    for bomb in bombs:
        (r, c) = bomb.position
        if strength_map[(r, c)] != bomb.blast_strength:
            return False
        if bomb_life_map[(r, c)] != bomb.life:
            return False
        if int(bomb_moving_direction[(r, c)] == 0):
            if bomb.moving_direction is not None and bomb.moving_direction != constants.Action(bomb_moving_direction[(r, c)]):
                return False
        elif constants.Action(bomb_moving_direction[(r, c)]) != bomb.moving_direction:
            return False
    return True
...
def update_agents(board, ammo, own_agent_id):
    ...
    for aid in [10, 11]:
        ...
        if len(locations[0]) != 0:
            agent.set_start_position((locations[0][0], locations[1][0]))
        else:
            agent.set_start_position((0, 0))
            agent.is_alive = False

        agent.reset(ammo=agent.ammo, is_alive=agent.is_alive)
