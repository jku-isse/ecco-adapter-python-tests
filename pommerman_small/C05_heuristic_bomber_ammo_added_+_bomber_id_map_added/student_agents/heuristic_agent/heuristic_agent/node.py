...
class Node(MCTSNode):
    ...
    def prune(self, actions):
        ...
        # prune place bomb if own_agent has no ammo
        if actions[own_agent.agent_id] == Action.Bomb.value & own_agent.ammo == 0:
            return True
        ...
        if self._is_legal_action(own_position, actions[self.agent_id], own_agent.ammo) and \
                self._is_legal_action(opponent_position, actions[opponent_agent.agent_id], 1):
            return False  # not prune actions
        return True

    def _is_legal_action(self, position, action, ammo):
...
def _value_func(state, root_state, agent_id):
    ...
    for bomb in state[2]:
        ...
        print(bomb.bomber)
        # TODO only check own bombs t = bomb.bomber does not work yet
        if bomb.bomber is own_agent:
            continue

        for i in range(1, bomb.blast_strength):  # init str = 2, then only 1 loop execution
            if loc[0]-i >= 0:
                tile = board[loc[0]-i][loc[1]]
        ...
        for i in range(1, bomb.blast_strength):
            if loc[0]+i < len(board):
                if board[loc[0]+i][loc[1]] == Item.Wood.value:
                    score += 0.05
                elif board[loc[0]+i][loc[1]] == Item.Rigid.value:
                    break
            else:
                break
        ...
        for i in range(1, bomb.blast_strength):
            if loc[1]-i >= 0:
                if board[loc[0]][loc[1]-i] == Item.Wood.value:
                    score += 0.05
                if board[loc[0]][loc[1] - i] == Item.Rigid.value:
                    break
            else:
                break

        for i in range(1, bomb.blast_strength):
            if loc[1]+i < len(board):
                if board[loc[0]][loc[1]+i] == Item.Wood.value:
                    score += 0.05
                if board[loc[0]][loc[1] + i] == Item.Rigid.value:
                    break
...
def _copy_bombs(bombs):
    ...
    for bomb in bombs:
        bombs_copy.append(
            characters.Bomb(bomb.bomber, bomb.position, bomb.life, bomb.blast_strength,
