...
def incr_position(action, col, row):
...
class Node(MCTSNode):
    def __init__(self, state, agent_id, reached_by_own_move):
        ...
        self.action_combinations = [(a1, a2) for a1 in POSSIBLE_ACTIONS for a2 in POSSIBLE_ACTIONS
                                    if not self.prune((a1, a2), self.state[0], reached_by_own_move)]

        self.children = dict()

    def prune(self, actions, board, reached_by_own_move):
        ...
        if man_dist > 6:  # farm phase
            if actions[opponent_agent.agent_id] != Action.Stop.value:
                # we do not model the opponent, if it is more than 6 steps away
                return True

            # do not go back during farm phase TODO not if picked up item?
            a = actions[own_agent.agent_id]
            if a in [1, 2, 3, 4] and REVERSE_MOVES[
                actions[own_agent.agent_id]] == reached_by_own_move:
                return True

            # if actions[own_agent.agent_id] == Action.Bomb.value:
            #     (x, y) = own_agent.position
            #     for i in (1, own_agent.blast_strength):
            #         if x + i <= 10 and board[x + i][y] == Item.Wood.value or \
            #                 0 <= x - 1 and board[x - i][y] == Item.Wood.value or \
            #                 y + i <= 10 and board[x][y + i] == Item.Wood.value or \
            #                 0 <= y - i and board[x][y - i] == Item.Wood.value:
            #             return False
            #     return True

                # prune stop move if agent has ammo
            if actions[own_agent.agent_id] == Action.Stop.value and own_agent.ammo > 0:
                return True
    ...
    def _is_legal_action(self, position, action, agent):
        ...
        col, row = incr_position(action, col, row)
        ...
        if board[row, col] == Item.Bomb.value:
            if not agent.can_kick:
                ...
                if row < 0 or row >= len(board) or col < 0 or col >= len(board):
                    return False
                if board[row, col] == Item.Passage.value:
                    return True

            # kick not allowed - no passage behind
            return False
...
def _value_func(state, root_state, agent_id, step, reached_by_own_move):
    ...
    if man_dist <= 6 and not opponent_agent.is_alive:
        return 2.5
    ...
    def islocked(x, y):
        down_cond = x + 1 >= len(board) or board[x + 1][y] not in ACCESSIBLE_TILES
        up_cond = x - 1 < 0 or board[x - 1][y] not in ACCESSIBLE_TILES
        right_cond = y + 1 >= len(board) or board[x][y + 1] not in ACCESSIBLE_TILES
        left_cond = y - 1 < 0 or board[x][y - 1] not in ACCESSIBLE_TILES
        return down_cond and up_cond and right_cond and left_cond

    if islocked(own_position[0], own_position[1]):
        score += -0.5

    if islocked(opponent_position[0], opponent_position[1]):
        score += 0.5

    # we want to push our agent towards the opponent
    if step > 400:
        score += 0.005 * (16 - max(4, man_dist))  # the closer to the opponent the better

    # we want to collect items (forward model was modified to make this easier)
    score += own_agent.picked_up_items * 0.25
    ...
    for bomb in state[2]:
        ...
        score += get_explosion_score(score_board, x, y, bomb_range, zeros, 11 - agent_id)
        score += get_explosion_score(score_board, x, y, -bomb_range, zeros, 11 - agent_id)
        score += get_explosion_score(score_board, x, y, zeros, bomb_range, 11 - agent_id)
        score += get_explosion_score(score_board, x, y, zeros, -bomb_range, 11 - agent_id)
    ...
    if board[x, y] != Item.Bomb.value:
        ...
        score += 0.2 * get_explosion_score(score_board, x, y, bomb_range, zeros, 11 - agent_id)
        score += 0.2 * get_explosion_score(score_board, x, y, -bomb_range, zeros, 11 - agent_id)
        score += 0.2 * get_explosion_score(score_board, x, y, zeros, bomb_range, 11 - agent_id)
        score += 0.2 * get_explosion_score(score_board, x, y, zeros, -bomb_range, 11 - agent_id)

    return score


def get_explosion_score(board, x, y, dx_arr, dy_arr, opponent_aid):
