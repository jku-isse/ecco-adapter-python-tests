...
class Node(MCTSNode):
    ...
    def _is_legal_action(self, position, action, ammo):
        ...
        #if board[row, col] in [Item.Bomb.value]: # and not canKick
        #    return False
...
def _value_func(state, root_state, agent_id):
    ...
    for bomb in state[2]:
        ...
        (x, y) = bomb.position
        bomb_range = np.arange(1, bomb.blast_strength + 1)
        zeros = np.zeros(bomb.blast_strength, dtype=np.int32)

        # only calculate for own bombs
        if bomb.bomber.agent_id != own_agent.agent_id:
            continue

        score += get_explosion_score(board, x, y, bomb_range, zeros)
        score += get_explosion_score(board, x, y, -bomb_range, zeros)
        score += get_explosion_score(board, x, y, zeros, bomb_range)
        score += get_explosion_score(board, x, y, zeros, -bomb_range)
    return score
