...
class LearningAgent(agents.BaseAgent):
    def __init__(self, *args, **kwargs):
        ...
        data_path = pkg_resources.resource_filename('//remotestorageurl/pommerman', 'ppo1')
        model_file = os.path.join(data_path, 'model.pt')

        # loading the trained neural network model
        self.model = net_architecture.CNNActorCritic(board_size=11, num_boards=7, num_actions=6)
